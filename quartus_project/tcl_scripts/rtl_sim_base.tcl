...
if {![info exists env(SIM_DIR)]} {
	set sim [file join $projdir simulation]
...
# Directory from which files generated by qsys are loaded
set submodules [file join $projdir sys simulation submodules]
if {![file isdirectory $submodules]} {
	error "ERROR! Qsys Submodules source folder doesn't exist, it's impossible to continue"
} else {
...
if {![file isdirectory $verilog_source]} {
	error "ERROR! Verilog files source folder doesn't exist, it's impossible to continue"
} else {
...
if {![file isdirectory $tb_source]} {
	error "ERROR! Testbench files source folder doesn't exist, it's impossible to continue"
} else {
	puts "Testbench source folder found"
	puts "path: $tb_source"
}

# For storing library folders generated for the simulation
set build $sim/build
if {[file isdirectory $build]} {
	# Avoids conflicts
	file delete -force $build
	puts "old build directory cleaned"
}
file mkdir $build
puts "build directory created"
...
set iputf $projdir/simulation/questa/pulpino_qsys_test_iputf_libs
...
vlib $iputf/rsp_demux_008
vmap rsp_demux_008 $iputf/rsp_demux_008

vlib $iputf/rsp_demux
vmap rsp_demux $iputf/rsp_demux

vlib $iputf/cmd_mux_008
vmap cmd_mux_008 $iputf/cmd_mux_008
...
vlib $iputf/router_011
vmap router_011 $iputf/router_011

vlib $iputf/router_010
vmap router_010 $iputf/router_010
...
vlib $iputf/pio_out
vmap pio_out $iputf/pio_out

vlib $iputf/pio_in
vmap pio_in $iputf/pio_in
...
vlib $iputf/GPIO_EXTRA
vmap GPIO_EXTRA $iputf/GPIO_EXTRA

vlib $iputf/GPIO_0
vmap GPIO_0 $iputf/GPIO_0

vlib $iputf/DEBUG
vmap DEBUG $iputf/DEBUG
...
file copy -force $projdir/sw/mem_init/sys_onchip_memory2_0.hex ./
...
vlog -sv "$submodules/sys_mm_interconnect_1_rsp_demux_008.sv"                     -work rsp_demux_008                           
vlog -sv "$submodules/sys_mm_interconnect_1_rsp_demux.sv"                         -work rsp_demux                               
vlog -sv "$submodules/sys_mm_interconnect_1_cmd_mux_008.sv"                       -work cmd_mux_008                             
vlog -sv "$submodules/altera_merlin_arbitrator.sv"                                -work cmd_mux_008                             
...
vlog -sv "$submodules/sys_mm_interconnect_1_router_011.sv"                        -work router_011                              
vlog -sv "$submodules/sys_mm_interconnect_1_router_010.sv"                        -work router_010                              
...
vlog     "$submodules/sys_pio_out.v"                                              -work pio_out
vlog     "$submodules/sys_pio_in.v"                                               -work pio_in
...
#------------------- PIO -----------------------
vlog     "$submodules/sys_GPIO_EXTRA.v"                                           -work GPIO_EXTRA
vlog     "$submodules/sys_GPIO_0.v"                                               -work GPIO_0
vlog     "$submodules/sys_DEBUG.v"                                                -work DEBUG
